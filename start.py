...
async def handler(event):
   ...
   # await client.send_file(chat,r1["image_url"],caption = r1["title"])
    ...
    await client.send_message(chat, "support @ymtvofficial    "+" TITLE:"+r1["title"]+"   DESCRIPTION:"+r1["description"],file=r1["image_url"], buttons=markup)
...
async def handler(event):
    ...
    await client.send_message(chat," support @ymtvofficial   "+A["title"],buttons=markup)
    print(A)
    print(link)
